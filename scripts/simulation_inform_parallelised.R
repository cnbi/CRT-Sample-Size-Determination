...
rows_divided <- split(1:nrow_designN1, 1:ncluster)     # CHANGE THIS
# missing <- rows_divided[[1]][2:7]
# rows_divided <- split(missing, 1:ncluster)     # CHANGE THIS
clusterExport(cl, c("SSD_crt_inform", "results_folder", "design_matrixN1"))   #CHANGE THIS
...
run_simulation <- function(Rows) {
  for (Row in Rows) {
    ...
    ssd_results <- SSD_crt_inform(eff_size = design_matrixN1[Row, 2],
                                ...
                                ndatasets = 5000, rho = design_matrixN1[Row, 1], #change
...
run_simulation <- function(Rows) {
  for (Row in Rows) {
    ...
    ssd_results <- SSD_crt_inform(eff_size = design_matrixN2[Row, 2],
                                ...
                                ndatasets = 5000, rho = design_matrixN2[Row, 1], #change
