...
for (Row in 1:40) {
  # Actual simulation
  ssd_results_null <- SSD_crt_null_plots(eff_size = design_matrixN2[Row, 2],
  ...
  # Save resulst
  file_name <- file.path(paste0(results_folder, "/ResultPlotsN2Row", Row, ".RDS"))
  saveRDS(ssd_results, file = file_name)
}


# Finding n1 (i.e. fixed n2)
for (Row in 1:40) {
  # Actual simulation
  ssd_results_null <- SSD_crt_null_plots(eff_size = design_matrixN1[Row, 2],
  ...
  # Save results
  file_name <- file.path(paste0(results_folder, "/ResultPlotsN1Row", Row, ".RDS"))
  saveRDS(ssd_results, file = file_name)
}
...
for (i in seq(nrow.design)) {
  load(paste("ResultPlotsN2Row", i, ".Rdata", sep = ""))
  r <- r
...
design_null <- design_matrixN2[, -1]
design_null$ind <- seq(nrow_designN2)
...
ggplot(results_all[results_all$eff.size == 0.2, ], aes(y = med.BF10, x = n2.after, color = as.factor(rho), shape = as.factor(n1))) +
  geom_point() + geom_line() +
  scale_color_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") +
  labs(title = "Bayes Factor H1 vs H0", subtitle = "H0:Intervention=Control \nH1:Intervention>Control") +
  xlab("Number of clusters") + ylab("Bayes Factor") +
...
ggsave(filename = "n2Eta01.png", path = paste0(path, results_folder), width = 1070, height = 650, units = "px", device = 'png', dpi = 400)
...
ggplot(results_all[results_all$rho != 0.25,], aes(y = eta.BF10, x = n2.after, color = as.factor(n1), shape = as.factor(n1))) +
  ...
  labs(color = "Cluster \nsizes", shape = "Cluster \nsizes") +
  geom_hline(yintercept = 0.8, colour = "red", linetype = "dashed") + theme(legend.position = "bottom")
ggsave(filename = "n2Eta10.png", path = paste0(path, results_folder), width = 1070, height = 650, units = "px", device = 'png', dpi = 400)
...
ggplot(results_all, aes(y = eta.BF10, x = n2.after, color = as.factor(n1), shape = as.factor(n1))) +
  ...
  labs(title = "Proportion BF H0 vs H1", subtitle = "H0:Intervention=Control \nH1:Intervention>Control") +
  xlab("Number of clusters") + ylab("Proportion of BF > BF threshold") +
...
ggplot(results_all, aes(y = eta.BF10, x = n2.after, color = as.factor(n1), shape = as.factor(n1))) +
  ...
  scale_color_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") +
  facet_grid(rows = vars(rho), cols = vars(eff.size), labeller = label_both) +
  labs(title = "Proportion BF H1 vs H0", subtitle = "H0:Intervention=Control \nH1:Intervention>Control") +
  xlab("Number of clusters") + ylab("Proportion of BF > BF threshold") +
  geom_hline(yintercept = 0.8)
...
## PMP ------------------------------------------------------------------------
ggplot(results_all, aes(y = mean.PMP0.H0, x = n2.after, color = as.factor(n1),
                        shape = as.factor(n1))) +
  geom_point() + geom_line() +
  scale_color_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") +
  facet_grid(rows = vars(rho), cols = vars(eff.size), labeller = label_both) +
  labs(title = "PMP of H0 when H0 = TRUE", subtitle = "H0:Intervention=Control \nH1:Intervention>Control") +
...
ggplot(results_all, aes(y = mean.PMP1.H1, x = n2.after, color = as.factor(n1), shape = as.factor(n1))) +
  ...
  scale_color_brewer(palette = "Dark2") + scale_fill_brewer(palette = "Dark2") +
  facet_grid(rows = vars(rho), cols = vars(eff.size), labeller = label_both) +
  labs(title = "PMP of H1 when H1 = TRUE", subtitle = "H0:Intervention=Control \nH1:Intervention>Control") +
  xlab("Number of clusters") + ylab("Posterior Model Probabilities")
...
d <- data.frame(x = densities$x, y = densities$y)
dd <- with(densities, data.frame(x, y))
